...
def index():
    ...
    try:
        ...
        # Backups recentes
        recent_backups = Backup.query.order_by(Backup.backup_date.desc()).limit(20).all()

        # Total de backups
        total_backups = Backup.query.count()

        # Últimos erros
        last_errors = Backup.query.filter_by(status='failed').order_by(
...
def devices():
    ...
    return render_template('devices.html', devices=[d.to_dict() for d in all_devices])
